...
export default function GeneralThemes({ title }) {
  ...
  const handleClick = (id) => {
    switch (id) {
      ...
      case "battery":
        ...
        break;
      case "heart":
        handleCustomeSelect({ preset_id: "heart", caption: "inlove", type: "heart" });
        break;
      default:
        break;
    }
  ...
  const buttons = [
    { id: "default", icon: <span className="mr-1 font-semibold">Aa</span>, label: "Văn bản" },
    { id: "music", icon: <img src="./icons/music_icon.png" className="w-6 h-6 mr-1" />, label: "Spotify" },
    { id: "music_apple", icon: <img src="./svg/lcd-empty-logo.svg" className="w-5 h-5 mr-1" />, label: "Apple Music" },
    { id: "review", icon: <img src="./icons/star_icon.png" className="w-5 h-5 mr-1" />, label: "Review" },
    { id: "time", icon: <PiClockFill className="w-6 h-6 mr-1 rotate-270" />, label: formattedTime },
    {
      id: "weather",
      icon: (
        <img
    ...
    <div className="px-4">
      ...
      {popupVisible && (
        <div
          ...
          transition-all duration-500 ${popupActive ? "opacity-100" : "opacity-0"}`}
        ...
        >
          ...
          >
            ...
            <p className="text-xs mb-2">
              Caption nhạc chỉ hiển thị trên IOS (Android vẫn đăng và hiển thị nhưng chỉ IOS thấy)
            </p>
      ...
      {reviewVisible && (
        <div
          ...
          transition-all duration-500 ${reviewActive ? "opacity-100" : "opacity-0"}`}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center space-x-1 mb-2">
              {[1, 2, 3, 4, 5].map((star) => (
                <StarProgress
                  key={star}
                  size={24}
                  fillPercent={Math.min(100, Math.max(0, (reviewRating - (star - 1)) * 100))}
                />
              ))}
            </div>

            <div className="flex items-center justify-between mb-4">
              <span className="text-sm text-gray-600">{reviewRating.toFixed(1)} / 5</span>
              <input
                type="range"
                min={0}
                max={5}
            ...
            <input
              value={reviewText}
              onChange={(e) => e.target.value.length <= 24 && setReviewText(e.target.value)}
